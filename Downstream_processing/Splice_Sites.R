...
my_list <- list("M2_1" = M2_1, "M2_2" = M2_2, "M2_3" = M2_3, "M2_4" = M2_4, "M4_1" = M4_1, "M4_2" = M4_2, "M4_3" = M4_3, "M4_4" = M4_4)
...
Tot_splice_intersect <- intersect(M2s_intersect, M4s_intersect)

#diff between pops
Tot_splice_uniqM2s <- data.frame(setdiff(M2s_intersect, M4s_union))
names(Tot_splice_uniqM2s)[1] <- "Pos"
...
nrow(Tot_splice_uniqM2s)
nrow(Tot_splice_uniqM4s)


#merging diffs back with full dataset to get strand (V4) information.
#M2s
M2_1_merged <- merge(M2_1, Tot_splice_uniqM2s, by = "Pos")
M2_2_merged <- merge(M2_2, Tot_splice_uniqM2s, by = "Pos")
M2_3_merged <- merge(M2_3, Tot_splice_uniqM2s, by = "Pos")
M2_4_merged <- merge(M2_4, Tot_splice_uniqM2s, by = "Pos")

table(M2_1_merged$V4) #0s present for strandedness seem to represent non-annotated regions based on visual inspection, could also be DNA contamination so filtering out.


M2s_bound <- rbind(M2_1_merged, M2_2_merged, M2_3_merged, M2_4_merged)
...
#M2s - stranded
M2s_bound_stranded<- subset(M2s_bound, V4 != 0)

#M4s - stranded
M4s_bound_stranded <- subset(M4s_bound, V4 != 0)

#Setting up to filter on avg RC - must have at least 10
...
sub_Avg_BG1_genes <- subset(Avg_BG1_genes, Avg_BG1 > 10)
...
sub_Avg_AG2_genes <- subset(Avg_AG2_genes, Avg_AG2 > 10)

Final_Avg_genes <- merge(sub_Avg_BG1_genes, sub_Avg_AG2_genes, by = "X1")


#long to wide format conversion - sanity check, match nrow with table results from above.
M2s_stranded_wide <- reshape(M2s_bound_stranded, 
                             timevar = "new_clumn",
...
M2s_stranded_wide$intron_start <- do.call(paste, c(M2s_stranded_wide[my_df_start], sep = "_"))
...
M2s_stranded_wide_withGenes <- merge(gtf, M2s_stranded_wide, by = "intron_start")
M2s_stranded_wide_filtRC <- merge(M2s_stranded_wide_withGenes, Final_Avg_genes, by.x = "ID", by.y = "X1")#here's the actual filter by avg RC

length(unique(M2s_stranded_wide_filtRC$Pos))#num BG1 specific high quality splice variants
unique(M2s_stranded_wide_filtRC$ID)#genes containing the BG1-specific splice variants

       
M4s_stranded_wide_withGenes <- merge(gtf, M4s_stranded_wide, by = "intron_start")
M4s_stranded_wide_filtRC <- merge(M4s_stranded_wide_withGenes, Final_Avg_genes, by.x = "ID", by.y = "X1")
...
M2s_DataS2 <- M2s_stranded_wide_filtRC[,c(1,12,13,14,15,16,19,24,29,34)]
M2s_DataS2 <- cbind(Samp = "BG1", M2s_DataS2)
colnames(M2s_DataS2)<- c("Samp","Gene_ID","Junc_Name", "Scaffold", "Junc_Start", "Junc_Stop", "Strand", "UniqReadsMappingJunc_Samp1","UniqReadsMappingJunc_Samp2", "UniqReadsMappingJunc_Samp3", "UniqReadsMappingJunc_Samp4")
M2s_DataS2_uniq <- M2s_DataS2 %>% distinct(Samp, Junc_Name, .keep_all = TRUE)
...
For_DataS2 <- rbind(M2s_DataS2_uniq, M4s_DataS2_uniq)
write.table(For_DataS2, file = "Data_S2.txt", row.names = F)
