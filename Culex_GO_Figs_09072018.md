...
top_14_MF <- list(Parous_only[c(1:14),3],DGC_MF_grVpa[c(1:14),3], DGC_MF_paVma[c(1:14),3], DGC_MF_paVpip[c(1:14),3])

tot_data_MF <- list(Parous_only[c(1:nrow(Parous_only)),3],DGC_MF_grVpa[c(1:nrow(DGC_MF_grVpa)),3], DGC_MF_paVma[c(1:nrow(DGC_MF_paVma)),3], DGC_MF_paVpip[c(1:nrow(DGC_MF_paVpip)),3])


#Getting the top 14 BP categories
Parous_only_BP <- read.table("./data/GO_analysis_parous_all/parous_all_GO_Dist_Level3_first3cols_BP.txt", sep = "\t", header = T)
...
top_14_BP <- list(Parous_only_BP[c(1:14),3],DGC_BP_grVpa[c(1:14),3], DGC_BP_paVma[c(1:14),3], DGC_BP_paVpip[c(1:14),3])

tot_data_BP <- list(Parous_only_BP[c(1:nrow(Parous_only_BP)),3],DGC_BP_grVpa[c(1:nrow(DGC_BP_grVpa)),3], DGC_BP_paVma[c(1:nrow(DGC_BP_paVma)),3], DGC_BP_paVpip[c(1:nrow(DGC_BP_paVpip)),3])
...
top_14_CC <- list(DGC_CC_grVpa[c(1:14),3], DGC_CC_paVma[c(1:14),3], DGC_CC_paVpip[c(1:14),3])

tot_data_CC <- list(DGC_CC_grVpa[c(1:nrow(DGC_CC_grVpa)),3], DGC_CC_paVma[c(1:nrow(DGC_CC_paVma)),3], DGC_CC_paVpip[c(1:nrow(DGC_CC_paVpip)),3])
```
...
data2_MF <- as.data.frame(top_14_MF, col.names = c("Parous_only","DGC_MF_grVpa", "DGC_MF_paVma", "DGC_MF_paVpip"))
...
data2_BP <- as.data.frame(top_14_BP, col.names = c("Parous_only","DGC_BP_grVpa", "DGC_BP_paVma", "DGC_BP_paVpip"))
...
data2_CC <- as.data.frame(top_14_CC, col.names = c("DGC_CC_grVpa", "DGC_CC_paVma", "DGC_CC_paVpip"))
...
###### "transmembrane transporter activity" = "transmem transp act"
...
lbls <- gsub("transmembrane transporter activity", "transmem transp act", lbls)
...
###### "single-organism metabolic process" = "single-org metabol process"

``` r
lbls <- c(as.character(Parous_only_BP[c(1:14),2]), "Miscellaneous")
...
lbls <- gsub("single-organism metabolic process", "single-org metabol process", lbls)
lbls <- paste(lbls, Rounded_Percentages_BP$Parous_only) # add percents to labels
lbls <- paste(lbls,"%",sep="") # ad % to labels 
Parous_only_BP <- pie(Rounded_Percentages_BP$Parous_only, labels =lbls, main = "Top Biological Process GO Categories for Parous CAL1", radius = 0.9, cex = 0.75, cex.main = 0.75, tck=.2, init.angle = 120)
```
...
##### Cellular Component GO Comparison Charts - left in for the fun of it, but not used in final paper.
...
``` r
col_schema <- c("lavender", "purple", "blue", "deepskyblue", "lightblue", "grey", "orange","darkorange", "tan", "brown", "chocolate", "gold", "yellow3", "yellow", "lightyellow")
